...
export const deleteHelp = {
  title: `Позволяет удалять горячие клавиши, переменные и пользовательские настройки, подробнее ${clickableLink(
    '#help delete'
  )}`,
  description: `Команда ${clickableLink(
    '#delete'
  )} позволяет удалять горячие клавиши, переменные и пользовательские настройки.
Синтаксис:
...
const deleteRecord = str => {
  ...
  if (!localStorage[storage])
    return echoHtml(
      `Хранилище не найдено, набери ${clickableLink(
        '#delete'
      )} для получения доступных хранилищ. \n`
    );
  if (str[1] === 'all') {
  ...
  if (!recordsArr[record])
    return echoHtml(
      `Запись не найдена, набери ${clickableLink(
        `#delete ${storage}`
      )} для списка записей. \n`
    );
...
const deleteRecordsShow = storage => {
  ...
  const recordsArr = JSON.parse(localStorage[storage]);
  if (Object.keys(recordsArr).length === 0)
    return echoHtml(`В ${storage} нет записей`);
  let records = 'Список записей доступных для удаления: \n';
  for (let record in recordsArr) {
    records += `${deleteClickableLink(storage, record)} : ${
      recordsArr[record]
    } \n`;
  }
  echoHtml(records);
