...
export const settingsHelp = {
  title: `Установить или сбросить пользовательские настройки, подробнее ${clickableLink(
    '#help settings'
  )}`,
...
const setProperty = stringCmd => {
  ...
  switch (propertyValueType) {
    ...
    case 'boolean':
      if (
        !(String(stringCmd[1]) === 'true') &&
        !(String(stringCmd[1]) === 'false')
      )
        return echoHtml('Параметр должен быть true или false');
  ...
  if (save)
    return (localStorage.properties = JSON.stringify(propertiesStorage));
};
...
const propertyDelete = property => {
  ...
  echoHtml(
    `Параметр ${property} удален. После перезагруки страницы будет использовано значение "по умолчанию"`
  );
};
...
const propertiesCmd = value => {
  ...
  if (stringCmd.length > 2)
    return echoHtml('Проверьте правильность введенной команды');
